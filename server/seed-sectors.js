...
const seedSectors = async () => {
  try {
    ...
    const sectors = [
      ...
      // Automotive Sectors
      { name: 'Automotive Manufacturing', industry: 'Automotive', description: 'Vehicle production and assembly' },
    ...
    for (const sector of sectors) {
      try {
        ...
        if (error.code === 11000) {
          console.log(`⚠️ Sector "${sector.name}" already exists, skipping...`);
        } else {
    ...
    console.log(`✅ Seeded ${successCount} sectors successfully`);
