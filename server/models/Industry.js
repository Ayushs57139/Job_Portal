...
// Static method to get all industries with their subcategories
industrySchema.statics.getAllIndustries = async function() {
  return await this.find({ isActive: true }).select('name subcategories');
};

// Static method to get subcategories for a specific industry
industrySchema.statics.getSubcategories = async function(industryName) {
  const industry = await this.findOne({ name: industryName, isActive: true });
  return industry ? industry.subcategories : [];
};

// Static method to seed industries from the provided data
industrySchema.statics.seedIndustries = async function() {
  const industryData = {
  ...
  try {
    ...
    const industries = Object.entries(industryData).map(([name, subcategories]) => ({
      name,
      subcategories
    }));
    
    await this.insertMany(industries);
