...
const AdminConsultancyDetailsScreen = ({ navigation, route }) => {
  const { consultancyId, mode = 'view' } = route.params;
  ...
  const fetchconsultancyDetails = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/admin/consultancies/${consultancyId}`, { headers });
      const data = await response.json();
      
      setconsultancy(data.consultancy);
      setFormData({
        companyName: data.consultancy.companyName || '',
        email: data.consultancy.email || '',
        phone: data.consultancy.phone || '',
        contactPerson: data.consultancy.contactPerson || '',
        address: data.consultancy.address || '',
        city: data.consultancy.city || '',
        state: data.consultancy.state || '',
        country: data.consultancy.country || '',
        pincode: data.consultancy.pincode || '',
        website: data.consultancy.website || '',
        industry: data.consultancy.industry || '',
        sectors: data.consultancy.sectors || [],
        departments: data.consultancy.departments || [],
        designation: data.consultancy.designation || '',
        description: data.consultancy.description || '',
        foundedYear: data.consultancy.foundedYear || '',
        consultancySize: data.consultancy.consultancySize || '',
        profilePhoto: data.consultancy.profilePhoto || null,
        coverPhoto: data.consultancy.coverPhoto || null,
      });
      setJobs(data.jobs || []);
    } catch (error) {
      console.error('Error fetching consultancy details:', error);
      Alert.alert('Error', 'Failed to fetch consultancy details');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/admin/consultancies/${consultancyId}`, {
        method: 'PUT',
        headers,
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Failed to update consultancy');
      }

      Alert.alert('Success', 'consultancy updated successfully');
      setEditMode(false);
      fetchconsultancyDetails();
    } catch (error) {
      console.error('Error updating consultancy:', error);
      Alert.alert('Error', error.message || 'Failed to update consultancy');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleImagePick = async (type) => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const token = await AsyncStorage.getItem('token');
        const response = await fetch(`${API_URL}/admin/consultancies/${consultancyId}/upload-${type}`, {
          method: 'POST',
          headers: {
  ...
  const handleSendEmail = (type) => {
    Alert.alert(
      'Send Email',
      `Select email type to send to ${consultancy.companyName}:`,
      [
  ...
  const sendEmail = async (emailType) => {
    try {
      ...
      await fetch(`${API_URL}/admin/consultancies/${consultancyId}/send-email`, {
        method: 'POST',
        headers,
  ...
  const handleSuspend = async () => {
    Alert.alert(
      'Suspend consultancy',
      'Are you sure you want to suspend this consultancy?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Suspend',
          ...
          onPress: async () => {
            try {
              ...
              await fetch(`${API_URL}/admin/consultancies/${consultancyId}/suspend`, {
                method: 'POST',
                headers
              });

              Alert.alert('Success', 'consultancy suspended successfully');
              fetchconsultancyDetails();
            } catch (error) {
              console.error('Error suspending consultancy:', error);
              Alert.alert('Error', 'Failed to suspend consultancy');
            }
          }
        }
      ]
  ...
  const handleDuplicate = async () => {
    Alert.alert(
      'Duplicate consultancy',
      'Are you sure you want to create a duplicate of this consultancy?',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Duplicate',
          onPress: async () => {
            try {
              ...
              const response = await fetch(`${API_URL}/admin/consultancies/${consultancyId}/duplicate`, {
                method: 'POST',
                headers
              });
              ...
              if (response.ok) {
                Alert.alert('Success', 'consultancy duplicated successfully');
                navigation.navigate('AdminConsultancyDetails', { consultancyId: data.consultancy._id });
              } else {
                throw new Error(data.message || 'Failed to duplicate consultancy');
              }
            } catch (error) {
              console.error('Error duplicating consultancy:', error);
              Alert.alert('Error', 'Failed to duplicate consultancy');
            }
          }
        }
      ]
    ...
    return (
      ...
      >
        <View style={dynamicStyles.loadingContainer}>
          ...
          <Text style={dynamicStyles.loadingText}>Loading consultancy details...</Text>
        </View>
      </AdminLayout>
    );
  }
    ...
    return (
      ...
      >
        <View style={dynamicStyles.errorContainer}>
          ...
          <Text style={dynamicStyles.errorText}>consultancy not found</Text>
          <TouchableOpacity
    ...
    >
      <ScrollView style={dynamicStyles.container}>
        ...
        <View style={dynamicStyles.infoSection}>
          <Text style={dynamicStyles.companyName}>{consultancy.companyName}</Text>
        ...
        <View style={dynamicStyles.formContainer}>
          <Text style={dynamicStyles.sectionTitle}>consultancy Information</Text>
          
          <View style={dynamicStyles.formGroup}>
            <Text style={dynamicStyles.label}>consultancy Name *</Text>
            <TextInput
          ...
          <View style={dynamicStyles.formRow}>
            ...
            <View style={[dynamicStyles.formGroup, dynamicStyles.formGroupHalf]}>
              <Text style={dynamicStyles.label}>consultancy Size</Text>
              <TextInput
                style={[dynamicStyles.input, !editMode && dynamicStyles.inputDisabled]}
                value={formData.consultancySize}
                onChangeText={(text) => setFormData({ ...formData, consultancySize: text })}
                editable={editMode}
          ...
          <View style={dynamicStyles.formGroup}>
            ...
            <TextInput
              ...
              editable={editMode}
              placeholder="Enter consultancy description"
              multiline
              numberOfLines={5}
            />
          </View>
        ...
        <View style={dynamicStyles.jobsSection}>
          <View style={dynamicStyles.jobsHeader}>
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('AdminJobs', { consultancyId })}
            >
...
export default AdminConsultancyDetailsScreen;
