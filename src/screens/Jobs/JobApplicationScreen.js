...
const JobApplicationScreen = ({ route, navigation }) => {
  ...
  const [keySkillsOptions, setKeySkillsOptions] = useState([]);
  ...
  const loadDynamicOptions = async () => {
    try {
      const [skillsRes, titlesRes, rolesRes] = await Promise.all([
        fetch(`${api.baseURL}/skills?limit=500`),
        fetch(`${api.baseURL}/job-titles?limit=500`),
        fetch(`${api.baseURL}/job-roles?limit=500`),
      ]);
      const [skillsData, titlesData, rolesData] = await Promise.all([
        skillsRes.json(), titlesRes.json(), rolesRes.json(),
      ]);
      if (skillsData.success && skillsData.skills?.length) {
        setKeySkillsOptions(skillsData.skills.map(s => s.name || s.skill || s));
      }
      if (titlesData.success && titlesData.jobTitles?.length) {
        setJobTitleOptions(titlesData.jobTitles.map(t => t.title || t.name || t));
      }
      if (rolesData.success && rolesData.jobRoles?.length) {
        setJobRolesOptions(rolesData.jobRoles.map(r => r.role || r.name || r));
      }
    } catch (e) {
      console.warn('Failed to load dynamic options:', e.message);
    }
  };
  ...
  const handleDocumentPick = async () => {
    try {
      ...
      // Handle both old API (result.type === 'success') and new API (result.canceled)
      const isSuccess = result.canceled === false || result.type === 'success';
      if (isSuccess) {
        // New API returns assets array, old API returns directly
      ...
      <View style={styles.inputContainer}>
        ...
        {showJobRolesDropdown && (
          <View style={styles.dropdown}>
            ...
            {/* Dropdown Options */}
            <ScrollView style={styles.dropdownScroll} nestedScrollEnabled={true}>
              {(jobRolesOptions.length > 0 ? jobRolesOptions : jobTitleOptions)
                .filter(option => 
              ...
              {(jobRolesOptions.length > 0 ? jobRolesOptions : jobTitleOptions).filter(option => 
                option.toLowerCase().includes(jobRolesSearch.toLowerCase()) &&
                !formData.currentJobRoles.includes(option)
              ).length === 0 && (
                <View style={styles.noResultsContainer}>
