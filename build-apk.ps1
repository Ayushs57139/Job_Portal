# Build Android APK Script (Local Build)
...
if ($AndroidSdkPath -and (Test-Path $AndroidSdkPath)) {
    ...
    if (-not $androidSdkPath) {
        ...
        Write-Host "  .\build-apk.ps1 -AndroidSdkPath `"C:\path\to\your\android\sdk`"" -ForegroundColor Green
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        if (Test-Path $apkPath) {
            ...
            Write-Host "APK Size: $([math]::Round($fileSize, 2)) MB" -ForegroundColor Green
            Write-Host "`nYou can now install this APK on Android devices!" -ForegroundColor Cyan
            Write-Host "Transfer to device and install, or use: adb install $fullPath" -ForegroundColor Yellow
        }
    } else {
